  ...
  return (
    ...
    >
      ...
      <View style={{ position: "absolute", alignItems: "center" }}>
        <Text
          style={{ fontSize: 36, fontWeight: "800", color, letterSpacing: 2 }}
        >
          {mins}:{secs}
        </Text>
      </View>
...
function IconStudies({ color, size = 24 }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      ...
      />
      <Path
        d="M6 10.5 L6 16 C6 16 9 19 12 19 C15 19 18 16 18 16 L18 10.5"
        stroke={color}
        strokeWidth="1.8"
        strokeLinecap="round"
        fill="none"
...
function CodeTab({ theme, t }) {
  const { PRIMARY } = useTabColors(theme);
  ...
  const generateCode = useCallback(
    async (notes = includeNotes) => {
      setLoading(true);
      try {
        const res = await api.post("/api/patient/share-code", {
          includeNotes: notes,
        });
        ...
        Alert.alert(
          "Error",
          "Could not generate share code. Please try again.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const copyCode = () => {
    if (code)
      Share.share({
        message: `Your FocusApp share code: ${code}\n\n${SHARE_DOMAIN}/share/${code}`,
      });
  };
    ...
    >
      ...
      >
        {loading ? (
          ...
          <>
            <Text
              style={[styles.codeText, { color: expired ? "#ccc" : PRIMARY }]}
            >
            ...
            <View style={styles.brandRow}>
              <Svg width="20" height="20" viewBox="0 0 24 24">
                ...
                />
                <Path
                  d="M12 6 L16 8.5 L16 13.5 L12 16 L8 13.5 L8 8.5 Z"
                  fill={PRIMARY}
                  opacity="0.3"
                />
              </Svg>
      ...
      <View style={[styles.infoCard, { shadowColor: PRIMARY }]}>
        <Text style={styles.description}>
          {t.shareDescription ??
            "Secure access to your own report on this website:"}
        </Text>
        {code && (
      ...
      <View style={styles.toggleRow}>
        ...
        <Text style={[styles.toggleLabel, { color: PRIMARY }]}>
          {t.sharePersonalNotes ?? "Share personal notes"}
        </Text>
      </View>
      ...
      <View style={styles.timerCard}>
        <View style={styles.timerHeader}>
          ...
          <Text
            style={[styles.timerLabel, { color: expired ? "#ccc" : PRIMARY }]}
          >
            {expired
...
function ASRSTab({ theme }) {
  const { PRIMARY, NAVY, MUTED } = useTabColors(theme);
  const [answers, setAnswers] = useState({});
  const score = Object.values(answers).filter((v) => v >= 3).length;
  const completed = Object.keys(answers).length === ASRS_QUESTIONS.length;
    ...
    >
      <View style={smCard}>
        ...
        <Text style={{ color: MUTED, fontSize: 13, lineHeight: 18 }}>
          Adult ADHD Self-Report Scale — Part A. Answer based on how you have
          felt over the past 6 months.
        </Text>
      </View>

      {ASRS_QUESTIONS.map((q, qi) => (
        <View key={qi} style={smCard}>
          <Text
            style={{
              color: NAVY,
              fontSize: 13,
      ...
      {completed && (
        ...
        >
          ...
          >
            {score >= 4
              ? "Consistent with ADHD symptoms — discuss with your doctor"
              : "Below threshold for ADHD screening"}
          </Text>
        </View>
      )}
    </ScrollView>
...
const STUDIES = [
  {
    ...
    year: "2023",
    summary:
      "Comprehensive review of diagnostic criteria and treatment outcomes for adult ADHD, including medication efficacy and behavioral interventions.",
    tag: "Treatment",
    tagColor: "#4a7ab5",
  },
  {
    ...
    year: "2022",
    summary:
      "Multi-year study following adults on methylphenidate and amphetamine-based treatments, assessing quality of life and symptom control.",
    tag: "Medication",
    tagColor: "#22C55E",
  },
  {
    ...
    year: "2023",
    summary:
      "Meta-analysis showing strong correlation between sleep disruption and ADHD symptom severity across 47 studies.",
    tag: "Sleep",
    tagColor: "#7AABDB",
  },
  {
    ...
    year: "2024",
    summary:
      "Study demonstrating that daily mood and focus tracking apps improve treatment adherence and clinician communication.",
    tag: "Digital Health",
    tagColor: "#FBBF24",
  },
];

function StudiesTab({ theme }) {
  const { PRIMARY, NAVY, MUTED } = useTabColors(theme);
      ...
      {STUDIES.map((study, i) => (
        <View key={i} style={smCard}>
          ...
          >
            <View
              style={{
                backgroundColor: study.tagColor + "22",
                borderRadius: 12,
          ...
          <Text style={{ color: "#555", fontSize: 13, lineHeight: 19 }}>
            {study.summary}
          </Text>
        </View>
      ))}
...
export default function ShareScreen({ navigation }) {
  ...
  const tabs = [
    ...
    {
      key: "studies",
      label: t.shareTabStudies ?? "Studies",
      Icon: IconStudies,
    },
  ];
  ...
  return (
    <View
      style={[styles.root, { backgroundColor: theme.bgSecondary ?? "#F0F4F8" }]}
    >
      <LinearGradient
      ...
      <View style={{ flex: 1 }}>
        {activeTab === "code" && <CodeTab theme={theme} t={t} />}
...
infoCard: {
  ...
  timerCard: {
    ...
    borderRadius: 24,
    width: "100%",
    paddingVertical: 28, // ← was 20, more breathing room
